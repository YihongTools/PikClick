...
為 `YihongTools/PikClick` 提供可直接作為 GitHub 首頁的中英文 README，清楚傳達用途、限制、安裝、開發與 Roadmap，並將 Android Release 版本統一為 v2.1.1。

## 使用情境與功能需求

- 訪客進入 repository 時，應先看到品牌 Banner、用途、安全界線與最新版本。
- 使用者應能從 README 前往 v2.1.1 Release，辨認 APK 與 checksum 檔名並完成安裝。
...
- Given v2.1.1 建置，When 執行 `checksumReleaseApk`，Then 產出同版本 APK 與可驗證的 SHA-256。
...
| 風險 | 影響 | 可能性 | 降低方式 |
|---|---|---|---|
| GitHub v2.1.1 Release 尚不存在 | 中 | 高 | 只建立本機產物與連結，不宣稱遠端已發布 |
...
- v2.1.1 GitHub Release 已由維護者建立並上傳 APK／checksum；後續版本仍需重新驗證。
